...
const props = defineProps<{
  isOpen: boolean;
  folder: { // The folder object being edited
    id: number;
    folder_name: string;
...
const submitForm = async (event: FormSubmitEvent<Schema>) => {
  ...
  try {
    ...
    toast.add({ title: 'Folder Updated', description: `Folder "${event.data.folder_name}" has been updated.`, color: 'green' });
    emit('folderUpdated', updatedFolderData); // Emit event with the updated folder data from API
    handleClose(); // Close the modal
  } catch (error: any) {
    console.error('Error updating folder:', error);
    const errorMessage = error.data?.detail || error.message || 'Could not update folder.';
