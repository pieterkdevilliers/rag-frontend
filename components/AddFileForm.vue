<template>
  <div class="max-w-md mx-auto p-4">
    <UForm :state="state" @submit="handleAddFiles">
      <h2 class="text-2xl font-bold mb-6 text-center">Add New Files</h2>
      ...
      <div v-if="state.selectedFiles.length > 0" class="mb-4">
        ...
        <ul class="list-disc list-inside text-sm text-gray-600 dark:text-gray-400">
          <li v-for="file in state.selectedFiles" :key="file.name">{{ file.name }}</li>
        </ul>
      </div>
...
const handleAddFiles = async () => { // Changed event type to any or FormSubmitEvent for UForm
  ...
  try {
    ...
    const responseData = await $fetch<{ response: string; uploaded_files: any[] }>(
    ...
    toast.add({ title: 'Files Uploaded', description: successMessage.value, color: 'green' });
  } catch (error: any) {
