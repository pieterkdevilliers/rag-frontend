<template>
    <div class="max-w-md mx-auto p-4">
        ...
        <h2 class="text-2xl font-bold mb-4">Add New File</h2>
        <div class="mb-4">
...
if (!folder_id) {
  ...
  console.log('Request body for get-text-from-url:', requestBody);
    ...
    if (responseData && responseData.response === "success") {
      ...
      toast.add({ title: 'Success', description: `Text processed from URL: ${responseData.url}`, color: 'green' });
      ...
      throw new Error( (responseData as any)?.detail || 'Processing URL succeeded but returned an unexpected format.');
    }
    ...
    } else if (error.response && error.response._data) { // Fallback for some error structures
        // error.response._data might be a string or an object
        const errorDetail = typeof error.response._data === 'object' ? error.response._data.detail : error.response._data;
        errorMessage.value = `Error: ${errorDetail || error.message}`;
    }
     else {
      errorMessage.value = error.message || 'Unable to add file from URL. Please check the URL and try again.';
    }
  } finally {
    isLoading.value = false;
  }
