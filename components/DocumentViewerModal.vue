<template>
	<UModal
		...
		:ui="{ width: 'sm:max-w-3xl md:max-w-4xl lg:max-w-5xl' }"
	>
		<!-- Added UI for wider modal -->
		<UCard
			:ui="{
				ring: '',
		...
		>
			...
			<div class="p-1 min-h-[70vh] max-h-[80vh] overflow-y-auto">
				<!-- Adjusted padding and height for iframe -->
				<div
					v-if="isLoading && !pdfUrl"
					class="flex justify-center items-center h-full py-10"
				>
					<div>
						<svg
				...
				<div v-else-if="errorLoadingDocument" class="text-red-500 p-4">
					...
					<p class="text-sm mt-1">
						Please ensure the file exists and you have permission to
						view it. Try closing this viewer and trying again.
					</p>
				</div>
				<!-- IFRAME APPROACH -->
				<div v-else-if="pdfUrl" class="w-full h-[calc(70vh-2rem)]">
					...
					<p>
						<span
							>Your browser does not support embedded PDFs. You
							can
							<a
								:href="pdfUrl"
								target="_blank"
				...
				<div
					v-else-if="!isLoading && !props.fileIdentifier"
					class="p-4"
				>
...
const displayFilename = computed(() => {
	...
	if (lastUnderscoreIndex !== -1) {
		const potentialIdPart = props.fileIdentifier.substring(
...
watch(
	() => props.isOpen,
	async (newVal) => {
		// Make the watch callback async
		if (newVal && props.fileIdentifier && props.accountUniqueId) {
			console.log(
				`Modal opening for file: ${props.fileIdentifier}, account: ${props.accountUniqueId}`
			);
			...
			const constructedPdfUrl = `${config.public.apiBase}/files/view/${
				props.accountUniqueId
			}/${encodeURIComponent(props.fileIdentifier)}`;
			console.log(
				'Constructed PDF URL for iframe/fetch:',
				constructedPdfUrl
			);
			...
			try {
				...
				if (!headResponse.ok) {
					...
					if (headResponse.status === 404) {
						...
						errorDetail =
							'Access denied (403). You may not have permission to view this document.';
					}
				...
				console.error(
					'Error preparing PDF URL or pre-flight check:',
					error
				);
				errorLoadingDocument.value =
