<template>
  <div>
    <h2 class="text-lg font-bold">Chat Transcript for Session #{{ sessionId }}</h2>
    ...
    <div v-else-if="data && data.length > 0" class="mt-4 space-y-4 max-h-[60vh] overflow-y-auto p-1">
      ...
      <div v-for="message in data" :key="message.message_id">
        <div class="flex items-start gap-2.5">
          ...
          <UIcon 
            :name="message.sender_type === 'user' ? 'i-heroicons-user-circle' : 'i-heroicons-cpu-chip'" 
            class="w-6 h-6 text-gray-400" 
          />
          <div class="flex flex-col w-full max-w-[320px] leading-1.5 p-3 border-gray-200 bg-gray-100 dark:bg-gray-700 rounded-e-xl rounded-es-xl">
    ...
    <div class="flex justify-end pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
      <UButton color="gray" variant="ghost" @click="emit('close')">Close</UButton>
    </div>
  </div>
</template>
