...
const handleDeleteFile = async () => {
  ...
  try {
    await $fetch(`https://fastapi-rag-2705cfd4c41a.herokuapp.com/api/v1/files/${uniqueAccountId}/${file.id}`, {
      method: 'DELETE',
      headers: {
    ...
    toast.add({ title: 'File Deleted', description: `File "${file.file_name}" has been deleted.`, color: 'green' });
    emit('fileDeleted', file.id); // Emit event with file ID
  } catch (error: any) {
    console.error('Error deleting file:', error);
    const errorMessage = error.data?.detail || error.message || 'Could not delete file.';
