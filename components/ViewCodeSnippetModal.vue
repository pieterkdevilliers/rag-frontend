    ...
    <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
      ...
      <div class="p-4">
        <p class="text-sm text-gray-600 dark:text-gray-300 mb-1">
          Copy the snippet below and paste it into your website's HTML, just before the closing body tag.
        </p>
        <p class="text-sm text-red-500 dark:text-red-400 mb-3">
          <strong>Important:</strong> You will need to replace 'YOUR_WIDGET_API_KEY' in the snippet with the actual API Key you copied when this widget was created.
        </p>

        <div v-if="generatedSnippet" class="space-y-2">
          <pre class="bg-gray-100 dark:bg-gray-800 p-3 rounded-md overflow-x-auto"><code class="text-sm text-gray-700 dark:text-gray-200">{{ generatedSnippet }}</code></pre>
      ...
      <template #footer>
        <div class="text-right">
          <UButton @click="handleClose" label="Close" color="gray" variant="outline" />
        </div>
      </template>
    </UCard>
</template>
...
const props = defineProps({
  ...
  widget: {
    type: Object as () => ({ id: number; name: string; account_unique_id?: string; /* other widget props */ }), // Add account_unique_id if passed
    required: true,
  },
});
...
const copySnippetToClipboard = async () => {
  if (generatedSnippet.value) {
    try {
      ...
      toast.add({ title: 'Copied!', description: 'Snippet copied to clipboard.', color: 'green' });
    } catch (err) {
      console.error('Failed to copy snippet: ', err);
      toast.add({ title: 'Copy Failed', description: 'Could not copy snippet. Please copy it manually.', color: 'red' });
    }
  }
};
