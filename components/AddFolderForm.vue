<template>
	<div class="max-w-md mx-auto p-4">
		<form @submit.prevent="handleAddFolder">
			<h2 class="text-2xl mb-4">Add New Folder</h2>
			<div class="mb-4">
				<label
...
const handleAddFolder = async () => {
	try {
		const response = await fetch(
			`https://fastapi-rag-2705cfd4c41a.herokuapp.com/api/v1/folders/${uniqueAccountId}/${folder_name.value}`,
			{
				method: 'POST',
				headers: {
		...
		errorMessage.value =
			'Unable to add Folder. Please check your credentials.';
	}
};
</script>
